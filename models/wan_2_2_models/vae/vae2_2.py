    ...
    def encode(self, videos, chunk=1):
        try:
            with torch.amp.autocast(_device_type(self.device), dtype=self.dtype):
                ...
                outs = []
                video_chunks = torch.chunk(videos, chunks=chunk, dim=0)

                for batch in video_chunks:
                    if batch.numel() == 0:
                        continue
                    encoded = self.model.encode(batch, self.scale).float()
                    outs.extend(list(encoded))   # 保持返回 list[tensor] 的形式
